...
QuakeMDL.prototype.parseSimpleFrame = function(f, header) {
	...
	for (var j=0; j<header.num_vertices; ++j) {
		vertices[j] = {
			'vertex': [
				f.readInt8() * header.scale[0] + header.translate[0],
				f.readInt8() * header.scale[1] + header.translate[1],
				f.readInt8() * header.scale[2] + header.translate[2]
			],
...
QuakeMDL.prototype.createMesh = function(f, header, texCoords, triangles, skins) {
	...
	mesh.Texture = new SoftEngine.Texture("TODO", header.skin_width, header.skin_height);
	...
	for (var i=0; i<header.num_frames; ++i) {
		...
		for (var index = 0; index < header.num_vertices; index++) {
			var vxt = vertices[index];
			var x = vxt.vertex[0];
			var y = vxt.vertex[1];
			var z = vxt.vertex[2];

			var nx = QuakeMDL.anormals[vxt.normalIndex][0];
			var ny = QuakeMDL.anormals[vxt.normalIndex][1];
			var nz = QuakeMDL.anormals[vxt.normalIndex][2];
